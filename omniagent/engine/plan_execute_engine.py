...
4. **参数名必须使用标准名称** — file_path（不是 path）、action（不是 command）、content（不是 text）
5. **tool 字段必须是下方列表中的精确工具名** — 严禁发明或猜测工具名
6. **不需要工具的步骤** — tool 设为 null，如"汇总分析结果"、"输出最终结论"
7. **每步只做一个原子操作** — 不要把"创建5个文件"写成一个步骤，拆成5个步骤
...
### 本地项目（正确示例 — 路径来自 list_files 的实际输出）

假设 list_files 返回了 app.py、requirements.txt、src/main.py、src/utils.py，则规划如下：
```json
{"analysis":"分析本地项目结构和代码质量","steps":[
  {"id":1,"task":"列出项目目录结构","tool":"list_files","params":{"file_path":"D:/myproject","pattern":"**/*.py"}},
  {"id":2,"task":"读取入口文件 app.py","tool":"read_file","params":{"file_path":"D:/myproject/app.py"}},
  {"id":3,"task":"读取核心模块 src/main.py","tool":"read_file","params":{"file_path":"D:/myproject/src/main.py"}},
  {"id":4,"task":"基于实际代码汇总分析结果","tool":null,"params":{}}
]}
```
⚠️ 注意：步骤 2-3 的 file_path 是**具体的文件路径**（如 `D:/myproject/app.py`），不是"基于步骤1的输出"这种自然语言描述。

### ❌ 错误示例（绝对禁止）
```json
{"id":2,"task":"读取入口文件","tool":"read_file","params":{"file_path":"基于步骤1的输出"}}
```
`"基于步骤1的输出"` 不是合法的文件路径！read_file 无法理解这句话。

### GitHub 仓库（需先克隆）

```json
{"analysis":"克隆并分析 GitHub 仓库","steps":[
  {"id":1,"task":"克隆仓库到本地","tool":"command","params":{"action":"git clone https://github.com/owner/repo.git repo_local"}},
  {"id":2,"task":"列出克隆后的目录结构","tool":"list_files","params":{"file_path":"repo_local","pattern":"**/*.py"}},
  {"id":3,"task":"读取 README（路径来自仓库约定）","tool":"read_file","params":{"file_path":"repo_local/README.md"}},
  {"id":4,"task":"读取核心源码（从步骤2输出中选择真实路径）","tool":"read_file","params":{"file_path":"repo_local/src/main.py"}},
  {"id":5,"task":"基于实际代码进行总结分析","tool":null,"params":{}}
]}
```
⚠️ 步骤 4 的 `file_path` 必须从步骤 2 的 list_files 输出中选一个**实际存在的文件路径**填入。
...
def _validate_tool_params(tool: str, params: dict) -> dict:
    ...
    for key in path_params & set(params.keys()):
        ...
        # 3. 中文字符 + 长度检测：路径通常不含过多中文字符
        cjk_count = sum(1 for c in value if '一' <= c <= '鿿')
        if cjk_count > 5:
            return {
                "valid": False,
                "reason": f"参数 '{key}' 包含 {cjk_count} 个中文字符: '{value[:80]}'。这看起来是自然语言而非文件路径。请使用 list_files 输出的实际路径。",
            }
...
class PlanExecuteEngine:
    ...
    def _execute_step_with_llm(
        ...
        return self._verify_llm_file_claims(result, tracker)
