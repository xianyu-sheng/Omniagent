...
class PlanExecuteEngine:
    ...
    def _scout_from_history(
        ...
        """Scout 回退：当前消息没有目录路径时，从对话历史中提取。

        这解决了 follow-up 消息（如"更详细一些"）没有目录路径
        导致 Plan 阶段缺少真实文件列表的关键问题。
        """
        # 只对明显的 follow-up 消息启用回退
        if not self._is_followup(user_input):
            return None

        # 从对话历史中提取目录
        history = context.get_conversation_messages() if context else []
        for msg in reversed(history[-10:]):  # 最近 10 条
            content = msg.get("content", "") if isinstance(msg, dict) else ""
            if not content:
                continue
            target_dir = self._extract_directory(content)
            if target_dir:
                import os
                if os.path.isdir(target_dir):
                    logger.info(
                        f"Plan-Execute Scout(fallback): 从对话历史中提取目录 {target_dir}"
                    )
                    # 重新执行 scout
                    return self._scout(
    ...
    def _is_followup(text: str) -> bool:
        ...
        for pattern in followup_patterns:
            if re.search(pattern, text, re.I):
                return True
        # 额外条件：消息较短且不包含任务特征（follow-up 通常很简短且无具体任务）
        task_indicators = [
            ...
            r"\b[A-Z]:[\\/]",  # Windows 路径
            r"\b\w+\.(?:py|js|ts|json|yaml|yml)\b",  # 文件名
        ]
        has_task = any(re.search(p, text, re.I) for p in task_indicators)
        if len(text) < 30 and not PlanExecuteEngine._extract_directory(text) and not has_task:
            # 短消息 + 无目录路径 + 无任务特征 = 很可能是 follow-up
            return True
        return False
