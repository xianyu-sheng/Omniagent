...
分析项目时，你有严格的探索预算：
- **前 2-3 步**：用 list_files 了解项目根目录 + 一级子目录结构
- **第 4-7 步**：用 read_file 读取最核心的源文件（入口文件、主要模块、配置文件）
- **第 8 步起**：必须开始合成 final_answer，不要再探索新文件
- 如果你在第 8 步还在读 README/build脚本/配置文件，说明探索策略失败
...
_MIN_FINAL_ANSWER_LENGTH = 200  # 分析类 final_answer 最低字符数
_MIN_STRUCTURED_SECTIONS = 2    # 至少包含 2 个分析维度
...
def _check_hollow_answer(
    ...
    # ── 辅助信号 1: 长度过短 ──
    is_too_short = len(text) < _MIN_FINAL_ANSWER_LENGTH
    if is_too_short:
    ...
    if is_too_short:
        reasons.append(f"长度仅 {len(text)} 字 (需要 ≥{_MIN_FINAL_ANSWER_LENGTH})")
    ...
    if is_analysis_task:
        ...
        if section_count < _MIN_STRUCTURED_SECTIONS and substantive_indicators < 2:
...
"""
    ...
    def run(self, user_input: str, context: AgentContext | None = None) -> str:
        ...
        for i in range(self.max_iterations):
            ...
            if remaining <= 4 and i > 0 and not tracker.has_executions():
                # 还没用过工具 → 强制要求快行动
                hurry_msg = (
            ...
            elif remaining <= 4 and tracker.has_executions() and len(tracker.calls) >= 6:
            ...
            if final_answer and final_answer.strip():
                ...
                hollow_check = _check_hollow_answer(final_answer, user_input, tracker)
