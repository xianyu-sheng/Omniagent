...
我花了几个月从零写了一个终端 AI Agent（[OmniAgent](https://github.com/xianyu-sheng/omniagent)），不是为了跟 Claude Code 竞争，是为了把 Agent 核心机制**做透、看懂**。这篇文章分享 6 个关键设计决策。
...
上面 6 个设计决策**不绑定 OmniAgent**。无论你用什么框架——LangChain、CrewAI、自研——只要你的 Agent 需要：
