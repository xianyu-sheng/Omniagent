...
V4 默认启用思考模式。模型发起工具调用后，下一次请求必须带回该 assistant 消息的 `reasoning_content`、`tool_calls` 以及匹配 `tool_call_id` 的工具结果。Xenon v0.6.3 起在 ReAct 原生工具链和当前会话的后续轮次中保留这组协议消息，避免工具调用后丢失思考状态。
...
- [ ] **看 toolbar** — `💾` 后面的数字就是当前会话命中率；它会随提示词复用程度变化
