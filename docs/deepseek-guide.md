...
Xenon 提供四层缓存可见性与安全调优，全部基于本地确定性计算，**不额外消耗 LLM API**：
...
❯ /cache status
缓存状态: WARM
  实际命中率: 99.3% · 字段覆盖率: 100%

❯ /cache explain
最近一次缓存判定: WARM
  直接证据: API 返回 hit=3,072, miss=23
...
平局，不越过显式 `-m`、会话锁、能力或健康排序，也不会发送预热请求。
...
运行 `/cache status`。`N/A` 表示厂商没有返回缓存字段，不能据此判断为零命中；
`COLD` 表示还没有足够样本。

### Step 2：查看最近请求的直接证据

运行 `/cache explain`，核对模型、引擎、阶段、真实 hit/miss token，以及本地
Manifest 推断出的模型切换、工具契约变化、上下文压缩代次或稳定前缀变化。

### Step 3：运行确定性检查

运行 `/cache doctor`。Prompt Compiler 会报告固定 system 区中的日期/时间等动态
内容，但不会静默改写用户指令；工具 schema 与 response format 会确定性排序。

### Step 4：预览安全优化

运行 `/cache optimize --dry-run`。确认边界后可使用 `--apply`；如不希望路由使用
缓存亲和，随时执行 `/cache optimize --disable`。这两个操作都不会调用模型。
...
- [ ] **看到 cold / n/a？** — 输入 `/cache explain`，先区分无样本、无字段和真实未命中
- [ ] **命中异常？** — 输入 `/cache doctor` 查看缓存族、前缀和工具契约诊断
- [ ] **想安全调优？** — 先运行 `/cache optimize --dry-run`，确认后再 `--apply`
- [ ] **退出时看一眼** — `/exit` 会打印本次会话总共省了多少钱
- [ ] **需要压缩上下文？** — `/compact` 会开启新的缓存代次，先接受下一次可能 cold
...
- [Xenon Prompt Compiler 源码](../xenon/utils/prompt_compiler.py)
