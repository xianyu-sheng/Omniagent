...
资料调研会使用严格只读工具边界；即使问题背景提到“提交”或“推送”，只要本轮明确请求是“查一下/比较”，也不会克隆仓库或弹出写入确认。GitHub API 限流时会尝试公开网页降级，连续三次工具失败后则基于已有证据结束，不会持续空转。

`clone_repo`、`command` 和 `git` 等长操作在交互终端显示已耗时与超时上限，可随时按 `Ctrl+C` 取消。此类有副作用操作发生超时后不会自动重放。
