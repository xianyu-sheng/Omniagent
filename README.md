...
[![Tests](https://img.shields.io/badge/tests-1116%2B-brightgreen.svg)]()
[![HumanEval](https://img.shields.io/badge/HumanEval_Pass@1-88.4%25_(official)-success.svg)](https://github.com/openai/human-eval)
[![v0.5.4](https://img.shields.io/badge/version-0.5.4-orange.svg)](https://github.com/xianyu-sheng/omniagent/releases)

![OmniAgent terminal demo](docs/assets/terminal-demo.svg)

### 🎥 操作演示

<video src="https://github.com/xianyu-sheng/Omniagent/blob/ubutnu/docs/assets/demo.webm?raw=true" controls width="100%"></video>

> 👆 完整终端操作录屏（48MB WebM），含双线输入框、状态栏、斜杠命令、ReAct 工具调用等。

> 12 家模型商、8 种推理范式、MCP 协议、断路器、上下文压缩——一个终端 Agent 该有的工程机制，这里都有。
> 23K 行 Python，1116+ 测试，MIT 开源。**适合想深入理解 Agent 架构的开发者阅读、修改、二次开发。**

---

## 为什么选择 OmniAgent？

| 特性 | OmniAgent | Claude Code | Cursor | Copilot |
|------|-----------|-------------|--------|---------|
| 国内大模型支持 | ✅ 12+ 厂商 | ❌ | ❌ | ❌ |
| 多引擎架构 | ✅ ReAct/Plan/Reflection 等 8 种 | ❌ 单一 | ❌ | ❌ |
| 本地工具执行 | ✅ 25+ 工具 | ✅ | ❌ 只读 | ❌ 只读 |
| MCP 协议 | ✅ | ✅ | ❌ | ❌ |
| 开源 | ✅ MIT | ❌ | ❌ | ❌ |
| 空洞检测 | ✅ | ❌ | ❌ | ❌ |
...
**OmniAgent 不是** Claude Code 或 Cursor 的竞品。它不追求在 SWE-bench 上刷榜，也不试图说服你换掉现有的编程助手。
...
# 指定引擎范式
omniagent chat --mode react "帮我排查这个 bug"
```
...
│                   模型调度层                               │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐   │
│  │AutoRouter│ │ ModelPool│ │Benchmark │ │ 断路器   │   │
│  │ 难度感知 │ │ 权重调度 │ │ Fetcher  │ │ 健康追踪 │   │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘   │
│  ┌──────────────────────────────────────────────────┐   │
...
- **ModelPool**：加权模型池，支持优先级和 fallback
...
- **prompt_toolkit 输入**：Claude Code 风格双线输入框 + 命令/路径/模型名三级补全
- **Rich 渲染**：Markdown 面板、语法高亮代码块、OSC-8 可点击路径
- **状态栏**：实时显示模型、Token 用量、消息数、会话时长
- **斜杠命令**：38 条内置命令
...
### OmniAgent 和 Claude Code / Cursor / Copilot 有什么区别？

Claude Code、Cursor 和 Copilot 是面向**终端用户**的 AI 编程助手，目标是开箱即用、帮你写代码。OmniAgent 是面向**开发者**的 Agent 参考实现，目标是展示 ReAct、断路器、MCP、上下文压缩等机制的工程细节。你可以把它当作学习 Agent 架构的教科书，也可以把它作为二次开发的基座。

详见上方[对比表](#为什么选择-omniagent)。
